...
class PacketTunnelSettingsGenerator {
    ...
    func generateNetworkSettings() -> NEPacketTunnelNetworkSettings {
        ...
        if let dnsHTTPSURL = tunnelConfiguration.interface.dnsHTTPSURL, let dnsHost = dnsHTTPSURL.host {
//            let dnsIps = DNSResolver.resolveSync(endpoints: [dnsHTTPSURL])
            let dnsSettings = NEDNSOverHTTPSSettings(servers: [])
            dnsSettings.serverURL = dnsHTTPSURL
            networkSettings.dnsSettings = dnsSettings
        }
        if let dnsTLSServerName = tunnelConfiguration.interface.dnsTLSServerName {
            let dnsSettings = NEDNSOverTLSSettings(servers: [])
            dnsSettings.serverName = dnsTLSServerName
            networkSettings.dnsSettings = dnsSettings
        }
